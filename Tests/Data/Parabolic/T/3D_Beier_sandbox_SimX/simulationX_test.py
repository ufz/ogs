...
df_server = read_csv("initial.csv", delimiter=";", index_col=[0], dtype={"data_index": str})


def get_Tin(t):
    df_readfile = read_csv("readfile.txt", delimiter=";")
    time_list = df_readfile["time"].tolist() # prepare the time adjustment
    t = min(time_list, key=lambda x:abs(x-t)) # !!!makes simulation much slower!!! - time adjustment to nearest value in list in case time value isn't in the list (happens in Beier test)
...
class BC(OpenGeoSys.BHENetwork):
    def initializeDataContainer(self):
        ...
        t = 0  # 'initial time'
        data_col_1 = df_server["Tin_val"].tolist()  # 'Tin_val'; option: give array directly
