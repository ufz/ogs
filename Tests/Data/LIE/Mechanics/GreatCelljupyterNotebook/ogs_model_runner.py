...
class SingleOGSModel:
    ...
    def __init__(
        ...
        print(f"[DEBUG] output project file: {self.prj.prjfile}")
    ...
    def replace_curve(
        ...
        root = self.prj._get_root()
        parameterpath = "./curves/curve"
        parameterpointer = self.prj._get_parameter_pointer(root, name, parameterpath)
        self.prj._set_type_value(
    ...
    def run(self, times_string, PEE_load_values, material_name):
        try:
            ...
            self.prj.run_model(
                ...
                wrapper=wrapper,
                args=f"-o {self.out_dir} -m {self.meshes}",
            )
    ...
    def update_fracture_effective_stress(self, model_type, load_case):
        ...
        try:
            self.prj.replace_text(value, xpath=xpath)
        except Exception as e:
    ...
    def apply_materials_for_LIE(self, material_name, model_type="default"):
        try:
            ...
            self.prj.replace_parameter_value(name="E1", value=material["young_sample"])
            self.prj.replace_parameter_value(name="nu1", value=material["nu_sample"])
            self.prj.replace_parameter_value(name="E2", value=rubber["young_modulus"])
            self.prj.replace_parameter_value(name="nu2", value=rubber["poisson_ratio"])

            if model_type in ["HM1", "HM2a", "HM2b"]:
                rate = material["fluid"]["injectionFlowRate_Inlet"]
                self.prj.replace_parameter_value(
            ...
            if model_type in ["M1", "M2a", "M2b"]:
                for mid in [0, 1]:
                    self.prj.replace_phase_property_value(
                        mediumid=mid,
                        phase="Solid",
                        name="density",
            ...
            if model_type in ["HM1", "HM2a", "HM2b"]:
                ...
                for mid in mids:
                    ...
                    if mid != 3:
                        ...
                        self.prj.replace_medium_property_value(
                            mediumid=mid,
            ...
            if self.n_fracture_p_ncs > 0:
                if model_type in ["HM2a", "HM2b"]:
                    self.prj.replace_parameter_value(name="S_f", value=material["S_f"])

                self.prj.replace_parameter_value(name="Kn", value=material["k_n"])
                self.prj.replace_parameter_value(name="Ks", value=material["k_t"])
                self.prj.replace_parameter_value(
                ...
                if self.fracture_model_type == "CohesiveZoneModeI":
                    ...
                    self.prj.replace_parameter_value(name="Gc", value=material["Gc"])
                    self.prj.replace_parameter_value(
    ...
    def apply_materials_for_VPF(self, material_name, model_type="default"):
        try:
            ...
            if self.mesh_size:
                ...
                self.prj.replace_parameter_value(name="ls", value=ls)
                print("ls", ls)
