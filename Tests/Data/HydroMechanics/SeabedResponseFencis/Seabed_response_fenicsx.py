...
ax[0].spines["right"].set_color(
    "none"
)  # https://www.grund-wissen.de/informatik/python/scipy/matplotlib.html
ax[0].spines["top"].set_color(
    "none"
)  # https://www.grund-wissen.de/informatik/python/scipy/matplotlib.html
ax[1].spines["right"].set_color(
    "none"
)  # https://www.grund-wissen.de/informatik/python/scipy/matplotlib.html
ax[1].spines["top"].set_color(
    "none"
)  # https://www.grund-wissen.de/informatik/python/scipy/matplotlib.html
...
ax[0].legend()
ax[1].plot(
    compute_pressure_and_stresses(t, 0, y_2D)[1],
    -y_rel,
    color=colors[6],
...
ax[1].plot(
    compute_pressure_and_stresses(t, 0, y_2D)[2],
    -y_rel,
    color=colors[2],
...
ax[1].plot(
    compute_pressure_and_stresses(t, 0, y_2D)[3],
    -y_rel,
    color=colors[4],
...
ax[1].legend()
plt.savefig(
    "./" + name_result_folder + "/distribution_over_depth_over_time_analytical_2D.pdf",
    bbox_inches="tight",
    format="pdf",
    dpi=600,
)
...
plt.savefig(
    "./" + name_result_folder + "/pore_water_pressure_over_time_analytical_2D.pdf",
    bbox_inches="tight",
    format="pdf",
    dpi=600,
)
...
fig.tight_layout()
plt.savefig(
    "./" + name_result_folder + "/distribution_over_depth_analytical_2D.pdf",
    bbox_inches="tight",
    format="pdf",
    dpi=600,
)
...
Vue = basix.ufl.element(
    "Lagrange", cell_name, degree=2, shape=(gdim,)
...
n_reference_fa3 = ufl.as_vector(
    (0, 1)
)  # normal vector reference configuration top facet
dimension = "2D"
...
p_ref = fem.Constant(
    domain, 101325.0
)  # Pa #reference pressure - in this case standard pressure
...
u, p = ufl.split(
    U
)  # splitting the solution into displacement u and porewater pressure p
...
bc_m1 = fem.dirichletbc(
    u_y, bottom_dofs_uy, V.sub(0).sub(1)
)  # no movement in y direction of the bottom facet
bc_m2 = fem.dirichletbc(
    u_x, right_dofs_ux, V.sub(0).sub(0)
)  # no movement in x direction of the right facet
bc_m3 = fem.dirichletbc(
    u_x, left_dofs_ux, V.sub(0).sub(0)
)  # no movement in x direction of the left facet
...
file_results_u = XDMFFile(
    domain.comm, "./" + name_result_folder + "/seabed_" + nc + "_u.xdmf", "w"
)
file_results_p = XDMFFile(
    domain.comm, "./" + name_result_folder + "/seabed_" + nc + "_p.xdmf", "w"
)
file_results_stress = XDMFFile(
    domain.comm, "./" + name_result_folder + "/seabed_" + nc + "_stress.xdmf", "w"
)
...
for n, t_ in enumerate(time):
    ...
    p_wave.interpolate(lambda x: A_t.value * np.cos(((2 * np.pi) / L) * x[0]))
...
ax.set_ylabel("$y$ / $L$")
ax.set_xlim(-1.1, 1.1)
ax.legend(bbox_to_anchor=(0.65, 0.15))
...
for step, mesh in zip(i_steps, meshes_stress):
    ...
    if step != 10:
        ax.plot(sigma_yy / 0.1e5, (depth / 100) - 1, "o", color=colors[step])
...
for step, mesh_stress in zip(i_steps, meshes_stress):
    ...
    f_abs_sigma_yy, f_abs_sigma_xx, f_abs_sigma_xy = compute_abs_and_rel_stress_error(
