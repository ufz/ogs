...
#
# This benchmark simulates a 2D cross-sectional example that models excavation in rock in two days ($\text{1.728}\cdot\text{10}^5$ s) under two-phase flow conditions.
# The 2D domain is a cross-section perpendicular to the tunnel axis, with a size of $140 \times 140 \textrm{m}^2$; the coordinates of the tunnel centre are (0,-857) m; the tunnel radius is 6.5 m.
# Only one quarter of the domain is used for the simulation, which has a duration of 2.5 days.
# The mesh is generated by this script and is shown in the figure below:
#
# <center>
...
#
# Liquid (water) density depends linearly on the liquid pressure.
# Gas density follows the ideal gas law.
# Phase properties are as follows:
# | Property  | Value  | Unit  |
# |---|---|---|
...
#
#
# Simulations are run with purely elastic deformation and with elasto-plastic deformation.
# For the elasto-plastic case we use the modified Mohr–Coulomb constitutive law (Mohr–Coulomb Abbo-Sloan).
...
# The initial capillary pressure is $10^4$ Pa.
# The initial gas pressure is set from a hydraulic steady-state condition:
# $$
# p^g_0= 10^5 - 9.81\rho_L  y  \quad\text{(Pa)},
# $$
# with $\rho_L$ the liquid density and $y$ the vertical coordinate.
# The total initial stresses are:
#
...
#
# For the fluid field, there is no flux on the left, bottom, and right boundaries.
# On the left and bottom boundaries, the displacement in the normal direction is fixed and the tangential traction component increment is zero.
# On the right boundary, the traction increment is zero.
# The remaining boundary conditions are listed below:
...
# 0, \quad t \ge 1.728\cdot 10^5\, (\text{s})
# \end{cases}.
# $$
#
...
# %% [markdown]
# The following figures illustrate the distributions of displacements and stresses at the end of the simulation.
...
# %% [markdown]
# The following figures illustrate the distributions of displacement, horizontal stress, and equivalent plastic strain at the end of the simulation.
# In comparison with elastic deformation, displacement and stress exhibit noticeable changes.
...
# %% [markdown]
# The following figures illustrate the distributions of gas pressure, liquid pressure, capillary pressure and saturation at the end of the simulation.
# In contrast to elastic deformation, these variables are also modified.
...
# %% [markdown]
# The left figure below illustrates the equivalent plastic strain profile at the bottom boundary, while the right figure illustrates the equivalent plastic strain variations at selected bottom points.
# From the latter, we observe that the largest equivalent plastic strain occurs at the position closest to the tunnel surface.
...
# %% [markdown]
# The following figures present the profiles of horizontal stress, $\sigma_{xx}$, and vertical stress, $\sigma_{yy}$ along the bottom boundary.
# The stress variations near the tunnel are clearly significant.
...
# %% [markdown]
# The following figures show the profiles of gas pressure and liquid pressure along the bottom boundary.
# The application of the plastic model results in a substantial change in the gas pressure.
...
# %% [markdown]
# The following figures show the profiles of capillary pressure and saturation along the bottom boundary.
# Using the plastic model results in a slight change in the saturation.
...
# %% [markdown]
# The figures below illustrate the variations in horizontal displacement (left) and gas pressure (right).
# It is evident that the application of the plastic model leads to reductions in both parameters.
...
# %% [markdown]
# The figures below illustrate the variations in capillary pressure (left) and saturation (right).
...
#
# This benchmark is conducted using both elastic and elasto-plastic models and Taylor-Hood elements.
# The results demonstrate the influence of the elasto-plastic model on the simulation outcomes.
# To tailor the run for shorter computation time, a coarse mesh is generated and a large cohesion value is assigned to the plastic model to reduce non-linear iterations.
# Adjustments to the mesh element density and cohesion can be made directly within this notebook.
