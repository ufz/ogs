// SPDX-FileCopyrightText: Copyright (c) OpenGeoSys Community (opengeosys.org)
// SPDX-License-Identifier: BSD-3-Clause

#include <gtest/gtest.h>

#include <Eigen/Core>

#include "BaseLib/MPI.h"
#include "MathLib/LinAlg/LinAlg.h"

#if defined(USE_PETSC)
#include "MathLib/LinAlg/PETSc/PETScMatrix.h"
#include "Tests/MathLib/PETScSparsityPatternUtils.h"
#else
#include "MathLib/LinAlg/Eigen/EigenMatrix.h"
#endif

#include "MathLib/LinAlg/FinalizeMatrixAssembly.h"
#include "NumLib/NumericsConfig.h"

using namespace MathLib::LinAlg;
#if defined(USE_PETSC)
using PETScSparsityPatternUtils::denseBlockSparsityPattern;
using PETScSparsityPatternUtils::diagonalSparsityPattern;
#endif

namespace
{
template <class T_MATRIX>
void checkGlobalMatrixInterface(T_MATRIX& m)
{
    ASSERT_EQ(10u, m.getNumberOfRows());
    ASSERT_EQ(10u, m.getNumberOfColumns());
    ASSERT_EQ(0u, m.getRangeBegin());
    ASSERT_EQ(10u, m.getRangeEnd());

    m.setValue(0, 0, 1.0);
    m.add(0, 0, 1.0);
    m.setZero();

    Eigen::Matrix2d local_m;
    local_m << 1.0, 1.0, 1.0, 1.0;
    std::vector<GlobalIndexType> vec_pos(2);
    vec_pos[0] = 1;
    vec_pos[1] = 3;
    m.add(vec_pos, vec_pos, local_m);

    ASSERT_TRUE(finalizeMatrixAssembly(m));
}

#ifdef USE_PETSC  // or MPI
template <class T_MATRIX, class T_VECTOR>
void checkGlobalMatrixInterfaceMPI(T_MATRIX& m, T_VECTOR& v)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};

    ASSERT_EQ(3u, mpi.size);
    ASSERT_EQ(m.getRangeEnd() - m.getRangeBegin(), m.getNumberOfLocalRows());

    int const gathered_rows =
        BaseLib::MPI::allreduce(m.getNumberOfLocalRows(), MPI_SUM, mpi);
    ASSERT_EQ(m.getNumberOfRows(), gathered_rows);

    int const gathered_cols =
        BaseLib::MPI::allreduce(m.getNumberOfLocalColumns(), MPI_SUM, mpi);
    ASSERT_EQ(m.getNumberOfColumns(), gathered_cols);

    // Add entries
    Eigen::Matrix<double, 2, 2, Eigen::RowMajor> loc_m;
    loc_m(0, 0) = 1.;
    loc_m(0, 1) = 2.;
    loc_m(1, 0) = 3.;
    loc_m(1, 1) = 4.;

    std::vector<GlobalIndexType> row_pos(2);
    std::vector<GlobalIndexType> col_pos(2);
    row_pos[0] = 2 * mpi.rank;
    row_pos[1] = 2 * mpi.rank + 1;
    col_pos[0] = row_pos[0];
    col_pos[1] = row_pos[1];

    m.add(row_pos, col_pos, loc_m);

    MathLib::finalizeMatrixAssembly(m);

    // Test basic assignment operator with an empty T_MATRIX._A
    T_MATRIX m_c = m;
    // Test basic assignment operator with an initialized T_MATRIX._A
    m_c = m;

    // Multiply by a vector
    // v = 1.;
    set(v, 1.);

    const bool deep_copy = false;
    T_VECTOR y(v, deep_copy);
    matMult(m_c, v, y);

    ASSERT_EQ(sqrt(3 * (3 * 3 + 7 * 7)), norm2(y));

    // set a value
    m_c.set(2 * mpi.rank, 2 * mpi.rank, 5.0);
    MathLib::finalizeMatrixAssembly(m_c);
    // add a value
    m_c.add(2 * mpi.rank + 1, 2 * mpi.rank + 1, 5.0);
    MathLib::finalizeMatrixAssembly(m_c);

    matMult(m_c, v, y);

    ASSERT_NEAR(sqrt((3 * 7 * 7 + 3 * 12 * 12)), norm2(y), 1e-12);
}

// Rectanglular matrix
template <class T_MATRIX, class T_VECTOR>
void checkGlobalRectangularMatrixInterfaceMPI(T_MATRIX& m, T_VECTOR& v)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};

    ASSERT_EQ(m.getRangeEnd() - m.getRangeBegin(), m.getNumberOfLocalRows());

    int const gathered_rows =
        BaseLib::MPI::allreduce(m.getNumberOfLocalRows(), MPI_SUM, mpi);
    ASSERT_EQ(m.getNumberOfRows(), gathered_rows);

    int const gathered_cols =
        BaseLib::MPI::allreduce(m.getNumberOfLocalColumns(), MPI_SUM, mpi);
    ASSERT_EQ(m.getNumberOfColumns(), gathered_cols);

    // Add entries
    Eigen::Matrix<double, 2, 3, Eigen::RowMajor> loc_m;
    loc_m(0, 0) = 1.;
    loc_m(0, 1) = 2.;
    loc_m(0, 2) = 3.;
    loc_m(1, 0) = 1.;
    loc_m(1, 1) = 2.;
    loc_m(1, 2) = 3.;

    std::vector<GlobalIndexType> row_pos(2);
    std::vector<GlobalIndexType> col_pos(3);
    row_pos[0] = 2 * mpi.rank;
    row_pos[1] = 2 * mpi.rank + 1;
    col_pos[0] = 3 * mpi.rank;
    col_pos[1] = 3 * mpi.rank + 1;
    col_pos[2] = 3 * mpi.rank + 2;

    m.add(row_pos, col_pos, loc_m);

    MathLib::finalizeMatrixAssembly(m);

    // Multiply by a vector
    set(v, 1);
    T_VECTOR y(m.getNumberOfRows());
    matMult(m, v, y);

    ASSERT_NEAR(6. * sqrt(6.), norm2(y), 1.e-10);
}

#endif  // end of: ifdef USE_PETSC // or MPI

}  // end namespace

#if defined(USE_PETSC)

template <class T_MATRIX, class T_VECTOR>
void checkAddToDiagonalMPI(T_MATRIX& m, T_VECTOR& v)
{
    // Set diagonal entries that this rank owns.
    for (PetscInt i = 0; i < m.getNumberOfLocalRows(); ++i)
    {
        m.set(m.getRangeBegin() + i, m.getRangeBegin() + i, 1.0);
    }
    MathLib::finalizeMatrixAssembly(m);

    // Apply addToDiagonal
    m.addToDiagonal(5.0);
    MathLib::finalizeMatrixAssembly(m);

    // Verify via matMult with a vector of ones.
    // Each diagonal entry is 1.0 + 5.0 = 6.0 and off-diagonals are 0,
    // so the result vector is [6,6,...] and norm2 = sqrt(n * 6^2).
    set(v, 1.);
    T_VECTOR y(m.getNumberOfRows());
    matMult(m, v, y);

    ASSERT_NEAR(std::sqrt(m.getNumberOfRows() * 36.0), norm2(y), 1e-12);
}

// PETScMatrix now always sizes itself locally — the global-size construction
// mode went away with PETScMatrixOption — so the tests below differ only in how
// many local rows and columns each rank gets, and in how much of that block the
// sparsity pattern fills. The former "_Global_Size" variants are kept as the
// padded case: more local rows than the assembly writes to, which is what
// exercises the explicit-zero fill of the preallocation.

TEST(MPI_Math, CheckInterface_PETScMatrix_Local_Size)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};
    MathLib::PETScMatrix A(2, denseBlockSparsityPattern(2, 2, 2, 2 * mpi.rank));

    const bool is_gloabal_size = false;
    MathLib::PETScVector x(2, is_gloabal_size);

    checkGlobalMatrixInterfaceMPI(A, x);
}

TEST(MPI_Math, CheckInterface_PETScMatrix_Padded_Local_Size)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};
    // 6 local rows per rank; only local rows 0 and 1 receive entries.
    MathLib::PETScMatrix A(6, denseBlockSparsityPattern(6, 2, 2, 6 * mpi.rank));

    // Vector must match the matrix's local column count (6 per rank).
    const bool is_global_size = false;
    MathLib::PETScVector x(6, is_global_size);

    checkGlobalMatrixInterfaceMPI(A, x);
}

TEST(MPI_Math, CheckInterface_PETSc_Rectangular_Matrix_Local_Size)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};
    MathLib::PETScMatrix A(2, 3,
                           denseBlockSparsityPattern(2, 2, 3, 3 * mpi.rank));

    const bool is_gloabal_size = false;
    MathLib::PETScVector x(3, is_gloabal_size);

    checkGlobalRectangularMatrixInterfaceMPI(A, x);
}

TEST(MPI_Math, CheckInterface_PETSc_Rectangular_Matrix_Padded_Local_Size)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};
    // 6 local rows, 9 local cols per rank; only local rows 0 and 1 receive
    // entries.
    MathLib::PETScMatrix A(6, 9,
                           denseBlockSparsityPattern(6, 2, 3, 9 * mpi.rank));

    // Vector must match the matrix's local column count (9 per rank).
    const bool is_global_size = false;
    MathLib::PETScVector x(9, is_global_size);

    checkGlobalRectangularMatrixInterfaceMPI(A, x);
}

TEST(MPI_Math, PETScMatrix_addToDiagonal_Local_Size)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};
    // 2 local rows per rank; preallocate each row's own diagonal entry.
    MathLib::PETScMatrix A(2, diagonalSparsityPattern(2, 2 * mpi.rank));
    const bool is_global = false;
    MathLib::PETScVector x(2, is_global);

    checkAddToDiagonalMPI(A, x);
}

TEST(MPI_Math, PETScMatrix_addToDiagonal_One_Row_Per_Rank)
{
    BaseLib::MPI::Mpi mpi{PETSC_COMM_WORLD};
    // 1 local row per rank, so each rank owns exactly its own diagonal. This
    // avoids cross-rank diagonal writes.
    MathLib::PETScMatrix A(1, diagonalSparsityPattern(1, mpi.rank));
    const bool is_global = false;
    MathLib::PETScVector x(1, is_global);

    checkAddToDiagonalMPI(A, x);
}

#else
TEST(Math, CheckInterface_EigenMatrix)
{
    MathLib::EigenMatrix m(10);
    checkGlobalMatrixInterface(m);
}

TEST(Math, EigenMatrix_addToDiagonal)
{
    MathLib::EigenMatrix m(4);
    // Set off-diagonal values
    m.setValue(0, 1, 2.0);
    m.setValue(2, 3, 3.0);

    // Apply addToDiagonal
    m.addToDiagonal(5.0);

    // Verify diagonal entries
    for (int i = 0; i < 4; ++i)
    {
        ASSERT_EQ(5.0, m.get(i, i));
    }
    // Off-diagonal unchanged
    ASSERT_EQ(2.0, m.get(0, 1));
    ASSERT_EQ(3.0, m.get(2, 3));
}
#endif
