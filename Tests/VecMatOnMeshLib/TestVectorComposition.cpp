...
TEST(TestTools, SeriesNumberGen)
{
	// test series() in TestTools.h
	// TODO this test should be moved to an appropriate file.
    std::vector<std::size_t> vec_numbers(10);
    std::generate(vec_numbers.begin(), vec_numbers.end(), series<std::size_t>());
    ASSERT_EQ(0u, vec_numbers[0]);
    ASSERT_EQ(1u, vec_numbers[1]);
    ASSERT_EQ(9u, vec_numbers[9]);
}
