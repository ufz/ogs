...
TEST(MaterialPropertyLib, RelPermBrooksCoreyNonwettingPhase)
{
    ...
    // s_L = 0.988 (s_eff ~= 0.998) lies in the interior band near full liquid
    // saturation where value() clamps k_rel to min_relative_permeability_; the
    // derivative must be zero there (the unclamped formula would give
    // ~-2.3e-5).
    std::array<double, 17> ref_saturation = {
        -0.01, 0.00, 0.01, 0.02, 0.04,  0.10, 0.20, 0.40, 0.60,
        0.80,  0.90, 0.96, 0.98, 0.988, 0.99, 1.00, 1.01};
