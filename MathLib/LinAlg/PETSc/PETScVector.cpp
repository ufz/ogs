...
void PETScVector::getGlobalEntries(PetscScalar u0[], PetscScalar u1[]) const
{
    ...
    std::copy(u1, u1 + _size_loc, global_buff+_start_rank);
    // Alternative for debugging:
    ...
    for(int i=0; i<_size_rank; i++)
    {
        if(i != _rank)
        {
            ...
            MPI_Sendrecv( u1, _size_loc, MPI_DOUBLE, i,tag,
                          u0,receive_size_loc, MPI_DOUBLE, i, tag, PETSC_COMM_WORLD, &status);
            for(int j=0; j<receive_size_loc; j++)
                global_buff[receive_start_rank+j] = u0[j];
        }
    }
