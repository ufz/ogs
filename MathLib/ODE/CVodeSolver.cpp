...
void CVodeSolverImpl::preSolve()
{
	...
	if (_f->hasJacobian())
	{
		    ...
		    ) -> int
		{
			...
			// See also the header files sundials_direct.h and cvode_direct.h
			// in the Sundials source code. The comments about the macro DENSE_COL
			// in those files indicate that matrices are stored column-wise.
			bool successful = fh->callJacobian(
			    t, NV_DATA_S(y), NV_DATA_S(ydot), DENSE_COL(jac, 0));
			return successful ? 0 : 1;
		};
