...
{
    DBUG("Create PhaseTransitionEvaporation as constitutive model.");
    ...
    n_components_gas_ = gas_phase.numberOfComponents();

    if (n_components_gas_ > 2)
    {
        OGS_FATAL(
            "More than two gas phase components are defined. Gaseous "
            "mixtures of more than two components are currently not "
            "provided.");
    }
    else if (n_components_gas_ < 2)
    {
        OGS_FATAL(
    ...
    // find the component for which the property 'vapour pressure'
    // is defined, because this corresponds to the evaporating component.
    for (int c = 0; c < n_components_gas_; c++)
    {
        if (gas_phase.component(c).hasProperty(
                MaterialPropertyLib::PropertyType::vapour_pressure))
        {
            gas_phase_vapour_component_index_ = c;
            break;
        }
    }

    // A lot of checks can (and should) be done to make sure that the
    // right components with the right properties are used. For example, the
    // names of the components can be compared to check that the name of the
    // evaporable component does not also correspond to the name
    // of the solvate.
    ...
    if ((gas_phase_vapour_component_index_ < 0) ||
        (gas_phase_dry_air_component_index_ < 0))
    {
        OGS_FATAL(
            "None of the components of the gas phase was assigned the "
