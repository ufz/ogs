...
PengRobinson::PengRobinson(Medium* m)
: _phase (0),
  _component (0)
{
...
PengRobinson::PengRobinson(Phase* p)
: _phase (p),
  _component (0)
{};
...
/// the Peng-Robinson EOS
PengRobinson::PengRobinson(Component* c)
: _phase (0),
  _component (c)
{};
...
PropertyDataType PengRobinson::value (VariableArray const&v)
{
    ...
    if (_phase)
    {
        ...
        const auto nComponents =
                std::min(std::max(1,
                        static_cast<int>(_phase->numberOfComponents())), 2);
