# -------------------------------------------------------------------
# Originally obtained from https://github.com/openmeeg/findmkl_cmake.
# Modified for OpenGeoSys in https://github.com/bilke/findmkl_cmake.
# -------------------------------------------------------------------
...
# -------
#
# Find a Intel® Math Kernel Library (Intel® MKL) installation and provide all
# necessary variables and macros to compile software for it.
#
# MKLROOT is required in your system
#
# we use mkl_link_tool to get the library needed depending on variables There
# are few sets of libraries:
#
# Array indexes modes:
#
# ::
#
# LP - 32 bit indexes of arrays ILP - 64 bit indexes of arrays
#
# Threading:
#
# ::
...
# library MPI support NOMPI - no MPI support INTEL - Intel MPI library OPEN -
# Open MPI library SGI - SGI MPT Library
#
# The following are set after the configuration is done:
#
# ::
#
# MKL_FOUND        -  system has MKL MKL_ROOT_DIR     -  path to the MKL base
# directory MKL_INCLUDE_DIR  -  the MKL include directory MKL_LIBRARIES    - MKL
# libraries MKL_LIBRARY_DIR  -  MKL library dir (for dlls!)
#
# Sample usage:
#
...
# ::
#
# find_package(MKL REQUIRED) if (MKL_FOUND)
# include_directories(${MKL_INCLUDE_DIR}) # and for each of your dependent
# executable/library targets: target_link_libraries(<YourTarget>
# ${MKL_LIBRARIES}) endif()

# NOTES
#
# If you want to use the module and your build type is not supported
# out-of-the-box, please contact me to exchange information on how your system
# is setup and I'll try to add support for it.
#
# AUTHOR
#
# Joan MASSICH (joan.massich-vall.AT.inria.fr). Alexandre GRAMFORT
# (Alexandre.Gramfort.AT.inria.fr) Théodore PAPADOPOULO (papadop.AT.inria.fr)
...
if(NOT MKL_ROOT_DIR)
    if(MKL_FIND_REQUIRED)
        message(
            FATAL_ERROR
                "Could not find MKL: please set environment or CMake variable MKLROOT or MKL_DIR!"
        )
    else()
    ...
    elseif(EXISTS "${MKL_ROOT_DIR}/tools/${MKL_LINK_TOOL_BINARY}")
        ...
        message(
            FATAL_ERROR
                "cannot find MKL link tool in any of the searched locations"
        )
    endif()
    ...
    if(FORCE_BUILD_32BITS)
        ...
        set(MKL_USE_interface "cdecl"
            CACHE STRING "disabled by FORCE_BUILD_32BITS" FORCE
        )
    else()
    ...
    if(MKL_USE_parallel)
        # With icc on linux, supported values by mkl_link_tool for threading
        # library are: iomp5|tbb
        if(WIN32)
    ...
    if(NOT ${COMMAND_WORKED} EQUAL 0)
        message(
            FATAL_ERROR
                "Cannot find the MKL libraries correctly. Please check your MKL input variables and mkl_link_tool. The command executed was:\n ${MKL_LINK_TOOL_COMMAND}."
        )
    endif()
    ...
    if(WIN32)
        ...
        if(MKL_LINK_TOOL_COMMAND MATCHES "static")
            string(REPLACE "$(MKLROOT)" "${MKL_ROOT_DIR}" MKL_LIBRARIES
                           ${MKL_LIBS}
            )
            # hack for lin with libiomp5.a
            if(APPLE)
                string(REPLACE "-liomp5"
                ...
                string(
                    REPLACE
                        "-liomp5"
                        "${MKL_ROOT_DIR}/../compiler/lib/${MKL_LIB_DIR}/libiomp5.a"
                        MKL_LIBRARIES
                        ${MKL_LIBRARIES}
                )
            endif()
            ...
            # get the lib dirs
            string(REGEX REPLACE "^.*-L[^/]+([^\ ]+).*" "${MKL_ROOT_DIR}\\1"
                                 INTEL_LIB_DIR ${MKL_LIBS}
            )
            if(NOT EXISTS ${INTEL_LIB_DIR})
                ...
                if(NOT EXISTS ${INTEL_LIB_DIR1})
                    message(
                        FATAL_ERROR
                            "MKL installation broken. Directory ${INTEL_LIB_DIR} does not exist."
                    )
                endif()
            ...
            if(EXISTS
               ${MKL_ROOT_DIR}/../../compiler/latest/linux/compiler/lib/${MKL_LIB_DIR}
            )
                list(
                    APPEND
                    MKL_LIBRARY_DIR
    ...
    # now definitions
    string(REPLACE "-libs" "-opts" MKL_LINK_TOOL_COMMAND
                   "${MKL_LINK_TOOL_COMMAND}"
    )
    execute_process(
        COMMAND ${MKL_LINK_TOOL_COMMAND}
        OUTPUT_VARIABLE RESULT_OPTS
        TIMEOUT 2
        ERROR_QUIET
    )
    ...
    find_package_handle_standard_args(
        MKL DEFAULT_MSG MKL_INCLUDE_DIR MKL_LIBRARIES
    )

    mark_as_advanced(
        MKL_INCLUDE_DIR MKL_LIBRARIES MKL_DEFINITIONS MKL_ROOT_DIR
        MKL_LINK_FLAGS
    )
endif()
