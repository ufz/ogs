 ...
 *  to the stiffness GlobalMatrix and right-hand side vector of a linear equation system
...
public:
     ...
     */
    virtual void computeResidual(GlobalMatrix const& M, GlobalMatrix const& K, GlobalVector const& b,
     ...
     * scheme.
     */
    virtual void pushMatrices(GlobalMatrix const& /*M*/, GlobalMatrix const& /*K*/,
...
/*! General GlobalMatrix translator used with time discretization schemes that have no special needs.
 *
 * \tparam GlobalMatrix the type of matrices occuring in the linearization of the equation.
 * \tparam GlobalVector the type of the solution vector of the equation
 * \tparam ODETag a tag indicating the type of equation.
 */
template<ODESystemTag ODETag>
...
/*! General GlobalMatrix translator for first order implicit quasi-linear ODEs, used with
...
private:
    ...
    GlobalMatrix& _M_bar; //!< Used to adjust matrices and vectors assembled by the ODE.
                    //!< \see pushMatrices()
