 ...
 *
 * \note This is an implementation detail of \c AndersonAcceleration, declared
 * in a separate header so that it stays out of the public class header while
 *       remaining reachable from the unit tests.
 *
 * \param G taken by value on purpose: it is normalized in place (divided by its
 *          largest diagonal entry) before the solve, so the caller's matrix
 * must not be touched.
 *
