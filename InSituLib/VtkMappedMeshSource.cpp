                                     ...
                                     vtkInformationVector *outputVector)
{
	...
	std::vector<std::string> propertyNames = properties.getPropertyVectorNames();

	// double
	for(std::vector<std::string>::const_iterator it = propertyNames.begin(); it != propertyNames.end(); ++it)
	{
		boost::optional<MeshLib::PropertyVector<double> const &> propertyVector(properties.getPropertyVector<double>(*it));
		if(!propertyVector)
			continue;
