...
public:
    ...
    GeoLib::Station::StationType getType() const
        { return static_cast<GeoLib::Station*>((*stations_)[0])->type(); };
...
//        { return GeoLib::readColorLookupTable(colorLookupTable_, filename); }

    /// Sets the stations as a vector
    void setStations(const std::vector<GeoLib::Point*>* stations) { stations_ = stations; }
...
protected:
    ...
    const std::vector<GeoLib::Point*>* stations_{nullptr};
    ...
    std::map<std::string, DataHolderLib::Color> colorLookupTable_;

private:
    std::size_t GetIndexByName( std::string const& name );

    std::map<std::string, vtkIdType> id_map_;
};
