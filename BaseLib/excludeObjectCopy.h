...
/// excludeObjectCopy copies only those elements that position within the source
/// vector is not in the exclude_positions vector. The positions in
/// exclude_positions have to be sorted in ascending order.
	...
	std::vector<std::size_t> const& exclude_positions)
{
	...
	for (std::size_t i=1; i<exclude_positions.size(); ++i) {
		std::copy_n(
			...
			exclude_positions[i]-exclude_positions[i-1],
			std::back_inserter(dest_vec)
		);
	}
	std::copy(src_vec.cbegin()+exclude_positions.back(),
